...
形式：
- SOP（*_sop.md）：用于以后可能重复执行的一类任务（如开发、浏览网页、检索文件、GUI/手机操作、发布验证等）的稳定做法；需满足曾经过试错/验证、包含非极简的「关键前置 + 典型坑」清单、下次同类任务可减少试错成本。SOP 记录跨任务可复用的流程/检查点/避坑，不记录某个具体项目或程序本次做了什么。避免长篇教程。
- 易变项目开发项/建模/业务口径/接口改造范围：若会随开发过程频繁变化、缺乏跨会话稳定复用价值，不写入 SOP 或 notes；需要时回到源码/当前文档重新确认。
