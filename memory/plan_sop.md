...
满足以下条件时，**不要默认进入 plan_mode，也不要启动探索 subagent**：

- 用户给出明确项目/文件/现象，且是 1~4 个具体 bug。
- 不涉及不可逆操作、数据库写入、跨仓库大重构或高风险权限操作。
- 可以用构建、测试、真实窗口/页面、日志或文件结果直接验证。

固定流程：

1. 写清验收条件（例如：哪个 exe/页面/接口，点击或命令后应该看到什么）。
2. 定向读取相关文件和日志，只围绕失败链路定位。
3. 做最小补丁，不扩大成重构计划。
4. 运行最贴近交付物的验证。
5. 验收通过后立即停止并汇报。

快速通道优先级高于后续 `[D]`/动态委托规则。只有连续 3 次有新证据仍无法定位，或影响范围扩大到大规模探索时，才升级为 plan/subagent。
...
- 所有执行步骤标记为 `[✓]`
- 验证方式按任务类型决定：
  - 编译/代码类：运行对应 build/test/check。
  - UI/桌面类：启动真实目标窗口，记录进程路径/窗口标题，并用截图/OCR/点击后状态验证。
  - Tauri release 类：验证 release exe、快捷方式目标、mtime、资源路径和真实窗口。
  - 文档/文件类：确认文件存在、关键内容可读且符合用户指定格式。
...
出现以下任一情况，必须立即停止继续读文件/重复定位：

- 连续 8 轮没有产生有效补丁、测试结果、构建结果或新的失败证据。
- 连续 3 次同类验证失败，且失败信息没有新增。
- summary 中反复出现“继续定位 / 准备补丁 / 不再空转 / 直接修复”，但实际没有改动或验证。

熔断后的唯一允许动作：

1. 输出当前已确认事实、失败证据和最可能根因。
2. 明确一个下一步动作：一次最小补丁、一次最小验证，或请求用户提供外部信息。
3. 若继续执行，下一轮必须产出补丁或新的验证证据；否则停止并汇报。
